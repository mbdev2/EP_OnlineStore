<?php
	...
	$idNarocilaQuery = mysqli_query($povezavaDoBaze, "SELECT * FROM narocila WHERE idStranke = '$idStranke' AND datumNarocila = '$datumNarocila'AND znesek = '$znesek'");
	...
	foreach ($naroceniIzdelki as $idArtikla=>$kolicina) {
		$trenutniNaroceniIzdelekQuery = mysqli_prepare($povezavaDoBaze, "INSERT naroceniIzdelki SET idNarocila = ?, idArtikla = ?, kolicina = ?");
